...
use E4u\Common\Exception\LogicException;
use E4u\Common\File;
...
class Image extends File
{
    ...
    public function getThumbnail($maxWidth = 0, $maxHeight = 0, $backgroundColor = 'ffffff', $forceOverwrite = false)
    {
        ...
        $filename = sprintf('%s-%dx%d-%s.%s',
            substr($this->getFilename(), 0, -1 - strlen($extension)),
            $size[0], $size[1], $backgroundColor, $extension);
        $thumbnail = new Image($filename, $this->getPublicPath());

        // create thumbnail file, if it does not exist already
        if ($forceOverwrite || !$thumbnail->fileExists()) {
            ...
            switch (strtolower($extension))
            {
                case 'jpg':
                case 'jpeg':
                    imagejpeg($out, $thumbnail->getFullPath(), static::JPEG_QUALITY);
                    break;
                case 'png':
                    imagepng($out, $thumbnail->getFullPath());
                    break;
                case 'gif':
                    imagegif($out, $thumbnail->getFullPath());
                    break;
            }
        }

        $this->_thumbnails[$_key] = $thumbnail;
        return $this->_thumbnails[$_key];
    }
}